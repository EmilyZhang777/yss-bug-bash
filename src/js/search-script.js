...
onReady(() => {
  ...
  mapProvider.load(secrets__YextMapsKeyGoogle(), {
    autocomplete: true,
    client: secrets__YextMapsClientGoogle(), // Remove this if not using Google
  }).then(() => {
    const locator = Composer.CobaltAce({
      cobaltBaseOpts: {
        mapProvider,
      },
    });
